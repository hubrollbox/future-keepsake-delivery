...
const PlanCard = ({ 
  ...
  return (
    <Card className={`emotion-card relative gentle-hover ${
      isPopular 
        ? 'border-primary/40 ring-2 ring-keepla-red/20' 
        : 'border-primary/20'
    }`}>
      {isPopular && (
      ...
      <CardHeader className="text-center pb-6">
        <CardTitle className="text-2xl font-fraunces text-muted-foreground-dark mb-2">{name}</CardTitle>
        <div className="space-y-1">
          <div className="flex items-center justify-center space-x-2">
            <span className="text-3xl font-bold text-primary">{price}</span>
            {originalPrice && (
              <span className="text-lg text-muted-foreground-light line-through">{originalPrice}</span>
            )}
          </div>
          <p className="text-muted-foreground-light text-sm">{description}</p>
        </div>
      </CardHeader>
