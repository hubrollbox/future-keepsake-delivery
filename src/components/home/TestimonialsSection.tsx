    ...
    <div className="mb-16 md:mb-20 animate-fade-in">
      ...
      <div className="grid md:grid-cols-3 gap-6 md:gap-8">
        {testimonials.map((testimonial, index) => (
          <Card key={index} className="emotion-card p-6 md:p-8 hover:shadow-soft hover:scale-105 transition-all duration-300 border border-dusty-rose/20 group">
            <CardContent className="pt-6">
              <div className="flex justify-center mb-4 md:mb-6">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star key={i} className="h-5 w-5 md:h-6 md:w-6 text-dusty-rose fill-current group-hover:animate-bounce-gentle" style={{ animationDelay: `${i * 0.1}s` }} />
                ))}
              </div>
              <p className="text-misty-gray italic mb-4 md:mb-6 leading-relaxed text-sm md:text-base">"{testimonial.text}"</p>
              <p className="font-semibold text-earthy-burgundy text-base md:text-lg">— {testimonial.name}</p>
            </CardContent>
          </Card>
        ))}
      </div>
